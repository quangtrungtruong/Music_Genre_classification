...
import sys
import librosa
import application.network.audio_processor as ap
import numpy as np
...
# for a song only
def extract_melgram(song_path, MULTIFRAMES):
    melgrams = np.zeros((0, 1, 96, 1366), dtype=np.float32)
    num_frames_total = list()
    if MULTIFRAMES:
        melgram = ap.compute_melgram_multiframe(song_path, MULTIFRAMES)
        ...
        return melgram, num_frames_total



# Melgram computation for multiple songs with several frames 
